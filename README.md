<details open>
  ...
  * When data is written to a particular partition, by defaul, data will be replicated into the other replicas. ```acks``` = 1 will ensure at least 1 broker is replicated, ```acks = all``` for all brokers to be replicated, ```acks = 0``` for ```jesus take the wheel```.
